...
def test_edgar_aapl_date_GD():
    ...
    try:
        ...
        assert isinstance(
            f, Filing
        ), f"Failed to open filing: should be Filing, but is {type(f)}"
        apple_city_facts = f.get_facts_by_concept_name("dei:EntityAddressCityOrTown")
        assert (
            len(apple_city_facts) == 1
        ), f"Expected 1 fact, but got {len(apple_city_facts)}"
        assert (
        ...
        assert (
            True
        ), f"Failed to download {uri}. The server responded with status code 403"
...
def test_edgar_input_BD():
    ...
    try:
        ...
        assert (
            "date" in str(e).lower()
        ), f"Expected ValueError for incorrect date format, but got {e}"
    except Exception as e:  # BAD PRACTICE, REMOVE IF POSSIBLE
        uri = "https://www.sec.gov/Archives/edgar/data/0000320193/000032019324000006/aapl-20231230_htm.xml"
        assert (
            True
    ...
    try:
        ...
        assert (
            "cik" in str(e).lower()
        ), f"Expected ValueError for incorrect cik format, but got {e}"
    except Exception as e:  # BAD PRACTICE, REMOVE IF POSSIBLE
        uri = "https://www.sec.gov/Archives/edgar/data/0000320193/000032019324000006/aapl-20231230_htm.xml"
        assert (
            True
    ...
    try:
        ...
        assert (
            "filing type" in str(e).lower()
        ), f"Expected ValueError for incorrect filing type, but got {e}"
    except Exception as e:  # BAD PRACTICE, REMOVE IF POSSIBLE
        uri = "https://www.sec.gov/Archives/edgar/data/0000320193/000032019324000006/aapl-20231230_htm.xml"
        assert (
            True
...
def test_edgar_BD():
    ...
    try:
        ...
        assert (
            False
        ), "Failed to raise ValueError for incorrect cik. The cik does not exist, but 0000000000 was given."
    except ValueError as e:
        assert (
            "cik" in str(e).lower()
        ), f"Expected ValueError for incorrect cik, but got {e}"
    except Exception as e:  # BAD PRACTICE, REMOVE IF POSSIBLE
        uri = "https://www.sec.gov/Archives/edgar/data/0000320193/000032019324000006/aapl-20231230_htm.xml"
        assert (
            True
