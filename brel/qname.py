...
class QNameNSMap:
    ...
    def add_to_nsmap(self, url: str, prefix: str):
        ...
        @raises ValueError: if the URL or the prefix is already in the
        namespace map, but mapped to a different prefix or URL
        """
        if prefix is None:
    ...
    def add_rename(self, rename_from: str, rename_to: str):
        ...
        if rename_to not in self.__prefix_to_url:
            raise ValueError(
                f"Invalid prefix rename: {rename_to}. The rename destination does not exist in the namespace map"
            )
        if rename_from not in self.__prefix_to_url:
            raise ValueError(
                f"Invalid prefix rename: {rename_from}. The rename source does not exist in the namespace map"
            )
