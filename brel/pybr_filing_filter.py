...
class FilingFilter:
    ...
    def __combine__piecewise(self, other: 'FilingFilter' | list[bool], operator) -> "FilingFilter":
        """
        ...
        if isinstance(other, list):
            other_filter = other
        elif isinstance(other, FilingFilter) and other.get_filter_type() == FilingFilterType.BOOL_LIST:
            other_filter = other.get_filter_list()
        else:
            raise TypeError("The other filter must be a list or a FilingFilter of type BOOL_LIST")
    ...
    @classmethod
    def make_aspect_filter(cls, facts: list[Fact], aspect: Aspect) -> "FilingFilter":
