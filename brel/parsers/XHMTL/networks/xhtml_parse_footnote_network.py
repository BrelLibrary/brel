...
from brel.parsers.utils.lxml_utils import get_elem_lang_recursive, get_str_attribute, get_str_attribute_optional
...
def parse_footnote(footnote_element: _Element, continuation_chain: List[_Element], taken_ids: Set[str]) -> BrelFootnote:
    ...
    if role is None:
        ...
        raise ValueError(f"Footnote with id '{id}' must have a valid URI for its footnoteRole. Got '{role}'.")
    ...
    content = extract_relevant_content_from_continuation_chain(footnote_element, continuation_chain)

    title = get_str_attribute_optional(footnote_element, 'title')
    return BrelFootnote(content, id, language, role, id, title)

def parse_footnotes(footnote_elements: List[_Element], continuation_chains: Dict[_Element, List[_Element]], taken_ids: Set[str]) -> Dict[str, BrelFootnote]:
    footnotes: Dict[str, BrelFootnote] = {}
    
    for footnote_element in footnote_elements:
        continuation_chain = continuation_chains.get(footnote_element)
        footnote = parse_footnote(footnote_element, continuation_chain, taken_ids)
        footnotes[footnote.get_id()] = footnote

    return footnotes

def parse_relationship(relationship_element: _Element, footnotes: Dict[str, BrelFootnote], fact_repository: FactRepository, taken_ids: Set[str]) -> Tuple[List[str], List[str], str, str, float]:
    ...
    if any([
        fact_repository.get_by_id_optional(toId) is None and
        toId not in footnotes
        for toId in toIds
    ]):
    ...
    if any([toId in footnotes for toId in toIds]) and not all([toId in footnotes for toId in toIds]):
        raise ValueError("Only footnotes or only facts should be referenced in toRefs. A mix of both was found.")
    ...
    if not arcrole:
        ...
        raise ValueError(f"Relationship with fromRefs '{fromRefs}' and toRefs '{toRefs}' must have a valid URI for its arcrole. Got '{arcrole}'.")
    ...
    if not linkrole:
        ...
        raise ValueError(f"Relationship with fromRefs '{fromRefs}' and toRefs '{toRefs}' must have a valid URI for its linkRole. Got '{linkrole}'.")

    order = get_str_attribute_optional(relationship_element, 'order')

    if order:
        try:
            order = float(order)
        except ValueError:
            raise ValueError(f"Relationship with fromRefs '{fromRefs}' and toRefs '{toRefs}' must have a valid floating-point value for its order. Got '{order}'.")
    else:
        order = 1.0
...
def create_footnote_network_nodes(
        arcs: List[Tuple[List[str], List[str], str, str, float]],
        footnotes: Dict[str, BrelFootnote],
        fact_repository: FactRepository
    ):
    # Compute Ids of root nodes
    all_ids = set(from_ids + to_ids for (from_ids, to_ids, _, _, _) in arcs)
    internal_ids = set(to_nodes for (_, to_nodes, _, _, _) in arcs)
    root_ids = all_ids - internal_ids

    id_to_node: Dict[str, FootnoteNetworkNode] = defaultdict(FootnoteNetworkNode)
    ...
    for (from_ids, to_ids, arcrole, linkrole, order) in arcs:
        ...
        for to_id in to_ids:
            if to_id not in id_to_node:
                to_object = footnotes[to_id] if to_id in footnotes else fact_repository.get_by_id(to_id)
...
def connect_footnote_network_nodes(arcs: List[Tuple[List[str], List[str], str, str, float]], id_to_node: Dict[str, FootnoteNetworkNode]):
...
def build_footnote_network(
    ...
    id_to_node, root_nodes = create_footnote_network_nodes(arcs, footnotes, fact_repository)
    connect_footnote_network_nodes(arcs, id_to_node)

    return FootnoteNetwork(root_nodes, linkrole, None, True)

def build_footnote_networks(arcs: List[Tuple[List[str], List[str], str, str, float]], footnotes: Dict[str, BrelFootnote], fact_repository: FactRepository) -> FootnoteNetwork:
    network_arcs: Dict[Tuple[str, str], List[Tuple[List[str], List[str], str, str, float]]] = defaultdict(list)
...
def parse_relationships(relationship_elements: List[_Element], footnotes: Dict[str, BrelFootnote], fact_repository: FactRepository, taken_ids: Set[str]) -> List[FootnoteNetwork]:
    arcs = []
    
    for relationship_element in relationship_elements:
        arcs.append(parse_relationship(relationship_element, footnotes, fact_repository, taken_ids))

    footnote_networks = build_footnote_networks(arcs, footnotes, fact_repository)
    return footnote_networks

def parse_role_reference(role_ref_element: _Element) -> str:
    link_type = get_str_attribute_optional(role_ref_element, 'xlink:linktype')
    if link_type != 'simple':
        raise ValueError(f"All (arc)role references must have linktype 'simple'. Got '{link_type}'.")
    ...
    if role != None and role == '':
        raise ValueError("If the xlink:role attribute is present in an (arc)role reference, it must not be empty.")

def parse_role_references(role_ref_elements: List[_Element], arcrole_ref_elements: List[_Element]) -> List[str]:
   for role_ref_element in (role_ref_elements + arcrole_ref_elements):
       parse_role_reference(role_ref_element)

def parse_footnote_networks_xhtml(context: FilingContext, footnote_network_elements: xhtml_footnote_network_elements) -> None:
    fact_repository = context.get_fact_repository()
    parse_role_references(footnote_network_elements.role_ref_elements, footnote_network_elements.arcrole_ref_elements)
