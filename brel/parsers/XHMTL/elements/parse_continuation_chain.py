...
from brel.parsers.utils.lxml_utils import get_prefix_localname_tag, get_str_attribute, get_str_attribute_optional
from brel.qnames.qname_utils import qname_from_str

def build_continuation_element_index(continuation_elements: list[_Element], taken_ids: Set[str]) -> None:
...
def create_continuation_chains(fact_elements: list[_Element], footnote_elements: list[_Element], continuation_elements: list[_Element], taken_ids: Set[str]) -> Dict[_Element, list[_Element]]:
    continuation_element_index: Dict[str, _Element] = build_continuation_element_index(continuation_elements, taken_ids)
    
    non_numeric_fact_elements = list(filter(
        lambda x: get_prefix_localname_tag(x.tag) == 'ix:nonNumeric',
        fact_elements)
    )
    ...
    for element in elements_which_can_continue:
        ...
        while continued_at != None:
            if continued_at in used_continuation_ids:
                raise ValueError(f"One of the IDs in the continuation chain of the fact or footnote with id '{get_str_attribute_optional(element)}' has already been used.")
        
            continuation = continuation_element_index.get(continued_at)
            if continuation is None:
                raise ValueError(f"One of the IDs in the continuation chain of the fact or footnote with id '{get_str_attribute_optional(element)}' does not exist.")
    ...
    for footnote_or_non_numeric, continuations in continuation_chains.items():
        ...
        for element in whole_fact_elements:
            for descendant in element.iterdescendants(qname_from_str('ix:continuation').clark_notation(), element.tag):
                if descendant in whole_fact_elements:
                    raise ValueError(f"The continuation chain must not contain any element which is a descendant of any other element in the same continuation chain")
    
        if len(continuations) == 0:
            raise ValueError(f"Fact or footnote with id '{get_str_attribute_optional(element)}' has no ix:continuation elements.")

    if len(continuation_elements) != len(used_continuation_ids):
        raise ValueError(f"Found {len(continuation_elements)} ix:continuation elements but only {len(used_continuation_ids)} were used.")
...
def extract_relevant_content_per_element(element: _Element) -> _Element:
    relevant_element = etree.Element(element.tag, element.attrib, element.nsmap)
    relevant_element.text = element.text
    relevant_element.tail = element.tail
...
def extract_relevant_content_from_continuation_chain(fact_element: _Element, continuation_chain: list[_Element]) -> _Element:
    head_content = extract_relevant_content_per_element(fact_element)
    tail_content = [extract_relevant_content_per_element(element) for element in continuation_chain]
