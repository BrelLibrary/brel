...
from brel.parsers.utils.lxml_utils import get_prefix_localname_tag, get_str_attribute_optional
from lxml.etree import _Element
from lxml import etree
from brel.contexts.filing_context import FilingContext

def parse_decimals_or_precision(str_value: str | None, is_decimals: bool) -> float | None:
    if str_value != None:
        try:
            ...
            raise ValueError(f"{'Decimals' if is_decimals else 'Precision'} should be an integer or 'INF'. Got {str_value} instead.")
...
def validate_descendant_non_fraction_rules(element: _Element) -> None:
    ...
    parent_tag = get_prefix_localname_tag(parent.tag)
    ...
    if xsi_nil:
        raise ValueError(f"Non-fraction fact cannot have xsi:nil attribute if it is a child of another non-fraction fact.")
    ...
    if this_format != parent_format or this_scale != parent_scale or this_unit != parent_unit:
        raise ValueError(f"Non-fraction fact cannot have different format, scale or unitRef attributes if it is a child of another non-fraction fact.")
    
def process_non_fraction_value(fact_value: str, format: str | None, scale: str | None, sign: str | None) -> str:
    if not format:
        value = None
        try:
...
def parse_non_fraction_fact_element(fact_element: _Element, context: Context, taken_ids=Set[str]) -> Fact:
    ...
    if decimals != None and precision != None or precision != None and xsi_nil != None or decimals != None and xsi_nil != None:
        raise ValueError("Fact cannot have more than one of: decimals, precision and xsi_nil attribute. Got more than one.")
    
    if xsi_nil and xsi_nil != 'true':
        raise ValueError(f"If xsi_nil is set, it should always be set to 'true'. Got {xsi_nil} instead.")
    ...
    if xsi_nil == 'true' and (has_text or has_children):
        raise ValueError(f"Non-fraction fact cannot have text or children if the xsi:nil attribute is set to 'true'.")

    if not xsi_nil and not has_text and not has_children:
        raise ValueError(f"Non-fraction fact cannot have neither text nor children if the xsi:nil attribute is not set to 'true'.")
    ...
    if sign and sign != '-':
        raise ValueError(f"If sign is set, it should always be set to '-'. Got {sign} instead.")
    ...
    if has_text:
        ...
        if len(element_children) > 1:
            raise ValueError("Non-fraction fact cannot have more than one child element if it has no text.")
        
        child_tag = get_prefix_localname_tag(element_children[0])
        if child_tag != 'ix:nonFraction':
            raise ValueError(f"Non-fraction fact cannot have a child element that is not ix:nonFraction. Got {child_tag} instead.")
        
        # Input whole element as fact value
        fact_value = etree.tostring(element_children[0])
