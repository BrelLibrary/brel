...
from brel.characteristics import UnitCharacteristic, ICharacteristic, BrelAspect
...
def parse_unit_from_xml(
    ...
    unit_id_str = xml_element.get("id")
    ...
    # turn the id into a qname
    if ":" in unit_id_str:
        unit_qname = make_qname(unit_id_str)
    else:
        unit_qname = make_qname(f"xbrli:{unit_id_str}")
    ...
    if len(children) != 1:
        raise ValueError(f"The unit {unit_qname} has {len(children)} children but should have 1 child")
    ...
    if "measure" in child_tag:
        ...
        if len(num_and_denom) != 2:
            raise ValueError(f"The unit {unit_qname} has {len(num_and_denom)} children but should have 2 children")
        ...
        for num_or_denom in num_and_denom:
            ...
            if "unitNumerator" in num_or_denom_tag:
                ...
                raise ValueError(f"The unit {unit_qname} has child {num_or_denom_tag} but should have children 'unitNumerator' and 'unitDenominator'")
        
        if num_and_denom[0].tag == num_and_denom[1].tag:
            raise ValueError(f"The unit {unit_qname} has two children with the same tag: {num_and_denom[0].tag}. One should be 'unitNumerator' and the other 'unitDenominator'")
    else:
        raise ValueError(f"The unit {unit_qname} has child {child_tag} but should have child 'measure' or 'divide'")
    
    # create the unit characteristic, add it to the cache and return it
    unit_characteristic = UnitCharacteristic(unit_qname, numerators, denominators)
