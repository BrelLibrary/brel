...
class PresentationNetworkFactory(IXMLNetworkFactory):
    ...
    def create_node(self, xml_link: lxml.etree._Element, xml_referenced_element: lxml.etree._Element, xml_arc: lxml.etree._Element | None, points_to: IReportElement|IResource) -> INetworkNode:
        ...
        return PresentationNetworkNode(points_to, [], arc_role, arc_qname, link_role, link_name, preferred_label_role, order)
    ...
    def update_report_elements(self, report_elements: dict[QName, IReportElement], network: INetwork) -> dict[QName, IReportElement]:
        ...
        for node in nodes:
            if isinstance(node.get_report_element(), Abstract):
                ...
                if parent is not None and isinstance(parent.get_report_element(), Hypercube):
                    nodes_to_promote.append(node)
                
                hypercube_children = list(filter(lambda x: isinstance(x.get_report_element(), Hypercube), node.get_children()))
