...
def check_duplicate_rolerefs(
    ...
    for xml_tree in xml_trees:
        ...
        roleref_parents.update(map(lambda roleref: roleref.getparent(), rolerefs))
    ...
    for roleref_parent in roleref_parents:
        ...
        if len(roleURIs) != len(set(roleURIs)):
            raise ValueError(f"the linkbase {roleref_parent} has duplicate rolerefs. All rolerefs must have a unique roleURI attribute.")
...
def check_roleref_pointers(
    ...
    for role_ref in role_refs:
        ...
        if role is None:
            raise ValueError(f"the roleref {role_ref} points to a non-existent role {href}")
...
def check_duplicate_arcs(
    ...
    for extended_link in extended_links:
        ...
        if len(arc_from_to) != len(set(arc_from_to)):
            raise ValueError(f"the link {extended_link} has duplicate arcs. All arcs must have a unique from and to attribute.")
