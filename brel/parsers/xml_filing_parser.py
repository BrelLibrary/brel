...
class XMLFilingParser(FilingParser):
    ...
    def __create_nsmap(self) -> None:
        ...
        qname_nsmap = self.get_context().get_nsmap()
    ...
    def parse_facts(self) -> None:
        ...
        xml_etrees = [
            self.__xml_repository.get_file(filename) for filename in xml_filenames
        ]

        parse_facts_xml(self.get_context(), xml_etrees)
